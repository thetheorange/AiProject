...
class ChatSql:
    ...
    def __init__(self):
        ...
        try:
            ...
            # print("这不是初始化了嘛？")
            self.add_mask("默认")  # 我觉得不用默认面具，只需要在前端完成就可以了
    ...
    # =============================================账户设置end=============================================
    # =============================================对话start=============================================
    def create_dialogue(self, name: str, mask_name: str = "默认"):
        """
        创建对话
        绑定账户id和面具id
        :param name: 对话名称
        ...
        try:
            with self.DB_session() as session:
                ...
                if mask_name == "" or not mask:
                    mask = session.query(Mask).filter_by(mask_name="默认").first()
    ...
    def get_dialogue_by_account_and_name(self, account_id: int, dialogue_name: str) -> Dialogue | None:
        """
        根据账户 ID 和对话名称获取对话
    ...
    def update_dialogue_by_account_and_name(self, account_id: int, dialogue_name: str)->None:
        ...
        :param dialogue_name: 对话名称
        """
        dialogue = self.get_dialogue_by_account_and_name(account_id, dialogue_name)
        if dialogue:
